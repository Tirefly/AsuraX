...
#include "K2Node/TireflyK2Node_GetAbilityParameter.h"
...
void UK2Node_GetAbilityParameter::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property->GetName() == GET_MEMBER_NAME_CHECKED(UK2Node_GetAbilityParameter, AbilityAssetSource).ToString())
	{
		BreakAllNodeLinks();
	...
	UFireflyGameplayAbilityAsset* AbilityAsset = GetAbilityAsset(OldAbilityAssetSourcePin);
...
void UK2Node_GetAbilityParameter::ExpandNode(FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	case ETireflyAbilityAssetSource::AbilityClass:
		{
			CallFunctionName = bPureNode ? GET_FUNCTION_NAME_CHECKED(UFireflyAbilitySystemLibrary, GetAbilityParamOfAbilityClassPure)
				: GET_FUNCTION_NAME_CHECKED(UFireflyAbilitySystemLibrary, GetAbilityParamOfAbilityClass);
			break;
		}
	case ETireflyAbilityAssetSource::AbilityId:
		{
			CallFunctionName = bPureNode ? GET_FUNCTION_NAME_CHECKED(UFireflyAbilitySystemLibrary, GetAbilityParamOfAbilityIdPure)
				: GET_FUNCTION_NAME_CHECKED(UFireflyAbilitySystemLibrary, GetAbilityParamOfAbilityId);
			break;
		}
	case ETireflyAbilityAssetSource::AssetRef:
		{
			CallFunctionName = bPureNode ? GET_FUNCTION_NAME_CHECKED(UFireflyAbilitySystemLibrary, GetAbilityParamOfAssetRefPure)
				: GET_FUNCTION_NAME_CHECKED(UFireflyAbilitySystemLibrary, GetAbilityParamOfAssetRef);
			break;
		}
	case ETireflyAbilityAssetSource::AssetPak:
		{
			CallFunctionName = bPureNode ? GET_FUNCTION_NAME_CHECKED(UFireflyAbilitySystemLibrary, GetAbilityParamOfAssetPakPure)
				: GET_FUNCTION_NAME_CHECKED(UFireflyAbilitySystemLibrary, GetAbilityParamOfAssetPak);
			break;
		}
	}
	...
	CallFunctionNode->FunctionReference.SetExternalMember(CallFunctionName, UFireflyAbilitySystemLibrary::StaticClass());
...
void UK2Node_GetAbilityParameter::ReallocatePinsDuringReconstruction(TArray<UEdGraphPin*>& OldPins)
{
	...
	UFireflyGameplayAbilityAsset* AbilityAsset = GetAbilityAsset(OldAssetSourcePin);
	FName Parameter = FName(OldParameterPin->DefaultValue);
	RecreateVariantPinsInternal(AbilityAsset, Parameter);
	...
	case ETireflyAbilityAssetSource::AbilityClass:
		{
			UEdGraphPin* Pin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Class,
				UFireflyGameplayAbility::StaticClass(), GetAbilityParameter_Helper::AbilityClassPinName);
			if (UBlueprint* Blueprint = FBlueprintEditorUtils::FindBlueprintForGraph(GetGraph()))
			{
				if (Blueprint->GeneratedClass.GetDefaultObject()->IsA(UFireflyGameplayAbility::StaticClass()))
				{
					Pin->PinName = UEdGraphSchema_K2::PN_Self;
				}
			}
	...
	case ETireflyAbilityAssetSource::AssetRef:
		{
			UEdGraphPin* Pin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object,
				UFireflyGameplayAbilityAsset::StaticClass(), GetAbilityParameter_Helper::AssetRefPinName);
...
void UK2Node_GetAbilityParameter::RecreateVariantPins()
{
	...
	UFireflyGameplayAbilityAsset* AbilityAsset = GetAbilityAsset();
...
void UK2Node_GetAbilityParameter::RecreateVariantPinsInternal(const UFireflyGameplayAbilityAsset* Asset,
...
UFireflyGameplayAbilityAsset* UK2Node_GetAbilityParameter::GetAbilityAsset(UEdGraphPin* Pin) const
{
	...
	UFireflyGameplayAbilityAsset* OutAsset = nullptr;
	switch (AbilityAssetSource)
	{
	case ETireflyAbilityAssetSource::AbilityClass:
		{
			if (AssetSourcePin->PinName == UEdGraphSchema_K2::PN_Self)
			{
				if (UBlueprint* Blueprint = FBlueprintEditorUtils::FindBlueprintForGraph(GetGraph()))
				{
					...
					{
						if (UFireflyGameplayAbility* Ability = BlueprintClass->GetDefaultObject<UFireflyGameplayAbility>())
						{
							OutAsset = Ability->GetAbilityAsset<UFireflyGameplayAbilityAsset>();
						}
					}
				}
			}
			else
			{
				...
				{
					if (UFireflyGameplayAbility* Ability = AbilityClass->GetDefaultObject<UFireflyGameplayAbility>())
					{
						OutAsset = Ability->GetAbilityAsset<UFireflyGameplayAbilityAsset>();
					}
				}
	...
	case ETireflyAbilityAssetSource::AbilityId:
		{
			...
			{
				OutAsset = AS->GetPrimaryAssetObject<UFireflyGameplayAbilityAsset>(FPrimaryAssetId(
					UFireflyGameplayAbilityAsset::GameplayAbilityAsset, FName(AssetSourcePin->DefaultValue)));
			}
			break;
		}
	case ETireflyAbilityAssetSource::AssetRef:
		{
			OutAsset = Cast<UFireflyGameplayAbilityAsset>(AssetSourcePin->DefaultObject);
			break;
		}
	case ETireflyAbilityAssetSource::AssetPak:
		{
			FSoftObjectPath AssetPath(AssetSourcePin->DefaultValue);
			OutAsset = Cast<UFireflyGameplayAbilityAsset>(AssetPath.TryLoad()); 
			break;
		}
	}
