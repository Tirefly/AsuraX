...
#define LOCTEXT_NAMESPACE "TireflyObjectPool"
...
void UK2Node_ActorPool_SpawnActor::AllocateDefaultPins()
{
	...
	// ActorClass pin
	CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Class, AActor::StaticClass(), FK2Node_ActorPool_SpawnActor_Helper::ClassPinName);
	...
	CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Struct, TransformStruct, FK2Node_ActorPool_SpawnActor_Helper::SpawnTransformPinName);
...
void UK2Node_ActorPool_SpawnActor::ExpandNode(FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	static const FName BeginSpawningBlueprintFuncName = GET_FUNCTION_NAME_CHECKED(UTireflyObjectPoolWorldSubsystem, ActorPool_BeginDeferredActorSpawn);
	static const FName FinishSpawningFuncName = GET_FUNCTION_NAME_CHECKED(UTireflyObjectPoolWorldSubsystem, ActorPool_FinishSpawningActor);
	...
	UClass* ClassToSpawn = GetClassToSpawn();
	UClass* SpawnClass = (TheClassPin != nullptr) ? Cast<UClass>(TheClassPin->DefaultObject) : nullptr;
	if (!TheClassPin || ((0 == TheClassPin->LinkedTo.Num()) && (SpawnClass == nullptr)))
	{
...
#pragma region BeginSpawn
	...
	CallBeginTheNode->FunctionReference.SetExternalMember(BeginSpawningBlueprintFuncName, UTireflyObjectPoolWorldSubsystem::StaticClass());
...
#pragma region FinishSpawn
	...
	CallFinishTheNode->FunctionReference.SetExternalMember(FinishSpawningFuncName, UTireflyObjectPoolWorldSubsystem::StaticClass());
...
UEdGraphPin* UK2Node_ActorPool_SpawnActor::GetExecThenPin() const
{
...
void UK2Node_ActorPool_SpawnActor::MaybeUpdateCollisionPin(TArray<UEdGraphPin*>& OldPins) const
{
	...
	for (UEdGraphPin* Pin : OldPins)
	{
			...
			|| Pin->PinName == FK2Node_ActorPool_SpawnActor_Helper::SpawnEvenIfCollidingPinName)
		{
			if (Pin->LinkedTo.Num() == 0)
			{
				...
				if (UEdGraphPin* const CollisionHandlingOverridePin = GetCollisionHandlingPin())
				{
