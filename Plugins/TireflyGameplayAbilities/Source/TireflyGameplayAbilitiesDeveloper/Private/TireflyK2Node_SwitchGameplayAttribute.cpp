...
UTireflyK2Node_SwitchGameplayAttribute::UTireflyK2Node_SwitchGameplayAttribute(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	FunctionName = TEXT("NotEqual_AttributeAttribute");
	FunctionClass = UTireflyAbilitySystemLibrary::StaticClass();
	OrphanedPinSaveMode = ESaveOrphanPinMode::SaveNone;
}

void UTireflyK2Node_SwitchGameplayAttribute::AddPinToSwitchNode()
{
	// 获取唯一的引脚名称
...
FName UTireflyK2Node_SwitchGameplayAttribute::GetUniquePinName()
{
	// 生成新的引脚名称，确保唯一性
...
FEdGraphPinType UTireflyK2Node_SwitchGameplayAttribute::GetPinType() const
{
	// 创建一个属性类型的引脚
	// Create a attribute type pin
...
FEdGraphPinType UTireflyK2Node_SwitchGameplayAttribute::GetInnerCaseType() const
{
	// 这个类型应该与 UTireflyAbilitySystemLibrary::NotEqual_AttributeAttribute 的第二个参数匹配！
	// This type should match the second argument of UTireflyAbilitySystemLibrary::NotEqual_AttributeAttribute !
	FEdGraphPinType PinType;
	PinType.PinCategory = UEdGraphSchema_K2::PC_String;
	return PinType;
}

FName UTireflyK2Node_SwitchGameplayAttribute::GetPinNameGivenIndex(int32 Index) const
{
	check(Index);
	return PinNames[Index];
}

void UTireflyK2Node_SwitchGameplayAttribute::CreateFunctionPin()
{
	// 在函数引脚上设置属性
...
void UTireflyK2Node_SwitchGameplayAttribute::CreateSelectionPin()
{
	// 获取默认的 K2 图谱模式
...
void UTireflyK2Node_SwitchGameplayAttribute::CreateCasePins()
{
	// 确保引脚名称数量不超过属性数量
...
void UTireflyK2Node_SwitchGameplayAttribute::RemovePin(UEdGraphPin* TargetPin)
{
...
void UTireflyK2Node_SwitchGameplayAttribute::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UTireflyK2Node_SwitchGameplayAttribute::GetTooltipText() const
{
	return NSLOCTEXT("K2Node", "SwitchGameplayAttribute_ToolTip", "Selects an output that matches the input value");
}

FText UTireflyK2Node_SwitchGameplayAttribute::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return NSLOCTEXT("K2Node", "Switch_GameplayAttribute", "Switch on Gameplay Attribute");
}

void UTireflyK2Node_SwitchGameplayAttribute::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	// 标志是否已修改
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UTireflyK2Node_SwitchGameplayAttribute, PinAttributes) || PropertyName == FName("AttributeName"))
	{
		bIsDirty = true;
	}
...
void UTireflyK2Node_SwitchGameplayAttribute::PostLoad()
{
	Super::PostLoad();
